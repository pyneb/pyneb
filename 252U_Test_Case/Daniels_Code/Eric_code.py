...
# spacing = 1
# minInd = np.argmin(LAP_action_array)

# fig, ax = plt.subplots(nrows=2)
# ax = ax.reshape((-1,1))
# for i in range(22):
#     ax[0,0].plot(np.arange(0,forces.shape[0],spacing),np.log10(np.abs(forces[::spacing,0,i])))
#     ax[1,0].plot(np.arange(0,forces.shape[0],spacing),np.log10(np.abs(forces[::spacing,1,i])))
    
# for a in np.ravel(ax):
#     a.axvline(minInd,color="black")
#TODO: suspect my issue was in computing the force as the gradient of the square
#root of the potential. compare by seeing what Eric's code does when I sub it out