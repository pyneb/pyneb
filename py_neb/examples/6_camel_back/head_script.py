...
def make_aux_func(func):
    def aux_func(coords):
        ...
        result= func(coords) + .2*np.sin(10*x)*np.sin(10*y)
        return(result)
    return(aux_func)
surface_name = "6_camel_back" # for output files
...
V_mod = V_func_shift#make_aux_func(V_func_shift)
EE = V_mod(np.array([xx,yy]))
...
### define initial path
#beginning point 
R0 = minima_coords[3]#np.array((-1.6,-.57))# NEB starting point
#end point
RN = minima_coords[2]#np.array((-0.1,0.71))#minima_coords[1]#(310,25) # NEB end point
...
LAP_params = {'potential':V_mod,'nPts':NImgs,'nDims':nDims,'mass':M_func,'endpointSpringForce': springForceFix ,\
...
for i,path in enumerate(allPaths_LAP):
    action_array_LAP[i] = utilities.TargetFunctions.action(path, V_mod,M_func)[0]
...
MEP_params = {'potential':V_mod,'nPts':NImgs,'nDims':nDims,'auxFunc':auxFunc,'endpointSpringForce': springForceFix ,\
...
for i,path in enumerate(allPaths_MEP):
    action_array_MEP[i] = utilities.TargetFunctions.action(path, V_mod,None)[0]   # endPointFix = (force_R0,force_RN) springForceFix
...
im = ax.contourf(grids[0],grids[1],EE,cmap='Spectral_r',extend='both',levels=MaxNLocator(nbins = 200).tick_values(-1,4))
ax.contour(grids[0],grids[1],EE,colors=['black'],levels=MaxNLocator(nbins = 15).tick_values(-1,4))  
